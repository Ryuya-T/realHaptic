...
\begin{document}
\begin{abstract}
A force/torqe is often requird to delect small  forse while its maximum load is also often large. In other words the dynamic range of force sensors should be higher.  In order to extend the dynamic range of the force/torque sensor, a high dynamic range force sensor that uses both a semiconductor strain gauge and a general strain gauge was developed. Experimenta results show that the dynamic range can be extended without complicating the structure of the strain body.\end{abstract}
\begin{jkeyword}
  力計測，力制御，ハプティクス
\end{jkeyword}
\begin{ekeyword}
